...
const newFormatNumber = () => {
  ...
  const number = input.val();
  const iDec = $("#qtdSel").val();  
  const formatedNumber = arFormat(Number(number), iDec);
  return $("#pNewRes").text(`Valor formatado com foi: ${formatedNumber}`);
};

const formatedJs = () => {
  const number = $('#num').val();
  const format = $('#format').val();
  $('.resFormat').text(format)
  let iDecs = 0;
  if(format === 'N2') {iDecs = 2} 
  const newNumber = arFormat(Number(number), iDecs)
  return $(".numFormated").text(`Valor formatado: ${newNumber}`);
}
